...
JAVA 导出窗口提供 **反编译并发数** 文本输入框，默认 `8`，可手动输入正整数。移除“自动 / 高并发 / 低内存”选择器；不再根据 IDEA 堆内存自动覆盖用户输入。实际工作线程数为输入值与批次数中的较小值；例如不足 40 个类族时只有一批，因此最多 1 个工作线程。

该数值控制 JarLib 调用反编译接口的并发量。底层反编译器如果还有限制（例如内置 Fernflower 的独立进程池和堆额度），实际同时反编译的数量可能更少，其余请求排队。`idea.log` 记录实际工作线程数和批大小。扫描索引、当前类族和 IDEA/VFS 仍占内存，但插件不累计已完成的源码。
...
性能对比可运行 `./gradlew test --tests '*ExportBenchmarkTest' -PbenchmarkExports=true`。测试启动真实 IDEA 测试应用，对 1600 个 class / 800 个类族的嵌套 JAR 比较串行、旧默认并发数、逐类族并行、40 类族分批并行、当前默认 8 并发以及 CPU × 2 并发；交替三轮并核对源码、相对路径及来源映射一致（报告行顺序不参与比较）。旧默认并发模式用于隔离并发数量差异，调度器均为当前实现。耗时仅报告，不作通过阈值。扫描对比继续由 `ScanBenchmarkTest` 覆盖。
