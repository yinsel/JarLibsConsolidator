...
先在项目视图右键执行 **一键添加依赖**，将要分析的 JAR/class 登记到 IDEA 依赖库；然后选择 **一键导出 CLASS（文件夹 / ZIP）** 或 **一键反编译并导出 JAVA（文件夹 / ZIP）**，输入白名单、黑名单，再选择导出父目录。插件自动创建 `<项目名>-classes` 或 `<项目名>-java-sources` 文件夹，沿用原 ZIP 的基本名称但不带 `.zip`；已有同名文件夹时使用 `-2`、`-3` 等后缀，不覆盖旧导出。
...
JAVA 导出完成后单独提示“反编译失败：N 个”。这里统计未生成源码的选中类版本，以及包含原生失败占位的源码文件；普通日志警告和扫描失败不混入该计数。N 大于 0 时，同一个文件夹额外包含 `decompilation-failures.csv`（UTF-8 BOM），列出完整类名、来源文件/JAR 条目、字节码 SHA-256、计划导出路径、异常类型和原因链；同名不同字节码分别记录。N 为 0 不生成该 CSV，CLASS 导出也不生成。CSV 支持逗号、双引号和多行异常；公式样式的字段会加前导单引号，完整原始消息仍保存在文本报告中。
...
写入失败会停止其他任务；取消会停止后续调度并等待工作线程退出，然后释放扫描临时文件。最终输出直接保留在所选文件夹，不再压缩打包。
