...
JAVA 导出调用 **当前 IDEA 自带的 Java Bytecode Decompiler（Fernflower）引擎**，不额外下载或内置第三方反编译器。需要启用 IDEA 的 Java Bytecode Decompiler 插件；关闭它时，JAVA 导出菜单隐藏，CLASS 导出仍可用。反编译会生成方法体，不使用仅含签名的 class 查看器存根。源码不保证与原始源码完全一致或能直接构建；同一来源、命中过滤的内部类（包括局部类和匿名类）与外部类合并成一个 `.java`，接近 IDEA 的展示方式。嵌套关系读取 JVM 元数据，不按 `$` 猜测；外部类缺失或被过滤时，内部类单独导出。不会为了合并读取被过滤的类。

从 1.5.5 起，单个类首次反编译失败或未生成源码时，会用同一 IDEA 引擎关闭泛型签名重建后重试一次。这用于处理内部类隐藏构造参数与泛型签名不一致等兼容问题；普通成功类不增加额外反编译。恢复结果保留方法体，但 `List<String>` 等泛型信息可能退化为原始类型，`export-report.txt` 会明确记录降级及首次失败原因，结果不进入无警告缓存。两次均失败仍列入失败报告，完整原因写入日志；取消操作不触发重试。重试仍只读取当前命中的类或类族，不读取或导出被过滤的外部类/内部类。
...
普通条目按包路径导出，例如 `com/example/User.class` 或 `com/example/User.java`。相同完整类名和相同字节内容去重；同名但内容不同的版本全部保留在 `classes-conflicts/来源名称--字节摘要/` 下，不会互相覆盖。JAVA 导出按整个类族的原始字节码判断版本：即使外部类字节相同，只要内部类内容不同，两套源码仍分别保留。反编译失败的版本会列入报告。
...
JAVA 导出完成后单独提示“反编译失败：N 个”。这里统计最终未完整生成源码的类版本；重试成功、普通反编译警告和扫描读取失败不混入该计数。N 大于 0 时，同一个 ZIP 额外包含 `decompilation-failures.csv`（UTF-8 BOM），列出完整类名、来源文件/JAR 条目、字节码 SHA-256、计划导出路径、异常类型和原因链；同名不同字节码分别记录。N 为 0 不生成该 CSV，CLASS 导出也不生成。CSV 支持逗号、双引号和多行异常；公式样式的字段会加前导单引号，完整原始消息仍保存在文本报告中。

从 1.5.7 起，两次尝试后仍有方法失败但引擎生成了部分源码时，会保留这些源码（包括健康的内部类），在文件顶部和完成提示中明确标记“部分源码”，并将失败类和方法签名记录到 CSV。部分源码不进入成功缓存，失败方法仍保留引擎的占位注释，不伪造实现。能识别失败成员时只统计该成员；无法识别或整个类族没有源码时，保守记录该类族的所有选中成员。首次与重试都不完整时，优先保留失败项较少的结果，数量相同则保留带泛型信息的首次结果。此行为保留可用的方法体，并不代表修复了 IDEA 引擎本身的反编译错误。

从 1.5.8 起，针对 IDEA 262 的参数自增反编译错误增加定向兼容重试：`PPandMMHelper` 在参数被分支重新赋值后，可能把参数与局部变量全局合并，导致前面的参数读取变成未定义变量，最终在 `SSAConstructorSparseEx.processExprent` 抛出空指针。仅匹配这一异常堆栈及确切失败方法后，将该方法的 `int` 参数 `IINC ±1` 在内存副本中改成等价的 `x - (-1)` / `x + (-1)`，避开错误的自增合并，再交给同一 IDEA 内置引擎。此变换保持整数溢出语义，不修改磁盘 class/JAR、导出来源哈希或 class 导出内容。正常类不增加重试；兼容处理最多重试一次，成功会记录在导出报告中，仍失败则保留原有部分源码和 CSV 记录。该处理不替换引擎，也不承诺修复其他类型的反编译错误。
...
从 1.5.6 起，JAVA 导出使用分批并行调度：每批 40 个类族（外部类及选中的内部类），批内顺序处理、批间并行。默认目标线程数为 CPU 核心数 × 2，同时按最大堆每 128 MiB 一个工作线程限制并发，并且不超过实际批次数；此调度估算不等于引擎分配内存的硬上限。每个类族仍只反编译一次并使用独立的 IDEA 引擎上下文；ZIP 按固定顺序由单线程写入。只提前提交与工作线程数相同数量的批次，完成结果的源码缓冲预算为 32 MiB，超出时暂存磁盘，消费后释放；取消会停止批内后续任务、等待工作线程退出并清理临时文件。该预算不包含引擎本身和下述独立会话缓存的内存。

同一次 IDE 会话内，成功且无警告的反编译结果保存在有界 LRU 内存缓存中：最多 4096 项、约 32 MiB 的输入/源码数据预算，重启或插件卸载后失效。缓存按完整类名和 SHA-256 定位，并再次比较原始字节内容；类族缓存包含所有选中成员的名称和字节，过滤变化或仅内部类变化也会失效；字节码变化会重新反编译。过滤在缓存查询前执行，失败/警告结果不会缓存。再次导出仍需扫描、校验字节码和打包，不会直接复用旧 ZIP。

性能对比可运行 `./gradlew test --tests '*ExportBenchmarkTest' -PbenchmarkExports=true`。该测试保留优化前实现，在同一 JVM 中对真实 JUnit JAR 和嵌套 Fat JAR 进行预热和交替三轮测试，记录旧版串行、新版首次导出和缓存命中后的中位耗时，在关闭内部类合并的兼容模式下检查所有源码及来源映射一致；另对 320 个 class 的类族合并测量首次/重复导出的耗时，检查 160 份外部类源码保留所有内部类方法体及 320 条来源映射。扫描对比可运行 `./gradlew test --tests '*ScanBenchmarkTest' -PbenchmarkExports=true`，以冻结的 1.5.1 实现对比 4000 类普通/嵌套 JAR 的全选和 1% 命中过滤，项目另有 12000 个无关文件。耗时不作为测试通过阈值；PR 构建会自动运行两项对比。
...
批调度对比也包含在 `ExportBenchmarkTest`：使用 1600 个 class / 800 个类族，对比旧调度、同线程数分批、CPU × 2 分批及缓存命中，轮换测试顺序并核对所有源码、条目顺序和来源映射一致。
