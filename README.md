...
JAVA 导出完成后单独提示“反编译失败：N 个”。这里统计最终未完整生成源码的类版本；整个类族失败时逐个记录其中被选择的成员（可能共享同一计划导出路径）；重试成功、普通反编译警告和扫描读取失败不混入该计数。N 大于 0 时，同一个 ZIP 额外包含 `decompilation-failures.csv`（UTF-8 BOM），列出完整类名、来源文件/JAR 条目、字节码 SHA-256、计划导出路径、异常类型和原因链；同名不同字节码分别记录。N 为 0 不生成该 CSV，CLASS 导出也不生成。CSV 支持逗号、双引号和多行异常；公式样式的字段会加前导单引号，完整原始消息仍保存在文本报告中。
