...
JAVA 导出保留每批 40 个类族（外部类及选中的内部类）的分批调度，批内顺序、批间并行。工作线程调用一次 IDEA 原生入口后直接写入对应 `.java` 文件，不等待整批完成，也不把源码放入 Future、结果队列或全量字符串列表。每批队列最多保留 40 个待消费结果，内容仅为警告/失败信息；不再使用旧的 32 MiB 源码缓冲及超额暂存后读回机制。来源报告、失败 CSV 持续写入，仅为完成对话框保留前 20 条提示和完整计数。

默认并发最多 2 个原生反编译任务，并按 CPU 数、最大堆进一步限制：为 IDEA 预留 512 MiB，每个任务估算 512 MiB；1 GiB 最大堆默认 1 个任务。该估算不是引擎内存的硬上限。反编译器仍需在内存中构建当前类族并返回完整源码，扫描索引及 IDEA/VFS 也占内存，因此无法做到零内存，但插件不会累计已完成的源码。降低并发优先控制内存，复杂类的实际耗时仍由原生引擎决定。
...
性能对比可运行 `./gradlew test --tests '*ExportBenchmarkTest' -PbenchmarkExports=true`。测试启动真实 IDEA 测试应用，对 1600 个 class / 800 个类族的嵌套 JAR 比较串行、逐类族并行、40 类族分批并行；交替三轮并核对源码、相对路径及来源映射一致。耗时仅报告，不作通过阈值。扫描对比继续由 `ScanBenchmarkTest` 覆盖。
