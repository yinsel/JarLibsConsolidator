...
### 导出 CLASS / 反编译导出 JAVA（文件夹）

先在项目视图右键执行 **一键添加依赖**，将要分析的 JAR/class 登记到 IDEA 依赖库；然后选择 **一键导出 CLASS（文件夹）** 或 **一键反编译并导出 JAVA（文件夹）**，输入白名单、黑名单，再选择导出父目录。插件自动创建 `<项目名>-classes` 或 `<项目名>-java-sources` 文件夹，沿用原 ZIP 的基本名称但不带 `.zip`；已有同名文件夹时使用 `-2`、`-3` 等后缀，不覆盖旧导出。
...
- **Jar Analyzer 并发**：使用 CPU 核数 × 2 个任务，与参考分支的常规线程池并发一致；需要为 IDEA 配置足够堆内存。
- **低内存**：单线程逐类族反编译并写入。
