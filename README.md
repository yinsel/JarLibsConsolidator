...
JAVA 导出保留每批 40 个类族（外部类及选中的内部类）的分批调度，批内顺序、批间并行。工作线程调用一次 IDEA 原生入口后直接写入对应 `.java` 文件，不等待整批完成，也不把源码放入 Future、结果队列或全量字符串列表。每批队列最多保留 1 个待消费结果，内容仅为警告/失败信息；不再使用旧的 32 MiB 源码缓冲及超额暂存后读回机制。来源报告、失败 CSV 持续写入，仅为完成对话框保留前 20 条提示和完整计数。
