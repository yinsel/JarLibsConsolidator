...
JAVA 导出完成后单独提示“反编译失败：N 个”。这里统计未生成源码的选中类版本，以及包含原生失败占位的源码文件；普通日志警告和扫描失败不混入该计数。N 大于 0 时，同一个 ZIP 额外包含 `decompilation-failures.csv`（UTF-8 BOM），列出完整类名、来源文件/JAR 条目、字节码 SHA-256、计划导出路径、异常类型和原因链；同名不同字节码分别记录。N 为 0 不生成该 CSV，CLASS 导出也不生成。CSV 支持逗号、双引号和多行异常；公式样式的字段会加前导单引号，完整原始消息仍保存在文本报告中。
...
从 1.5.6 起，JAVA 导出使用分批并行调度：每批 40 个类族（外部类及选中的内部类），批内顺序处理、批间并行。默认目标线程数为 CPU 核心数 × 2，同时按最大堆每 128 MiB 一个工作线程限制并发，并且不超过实际批次数；此调度估算不等于引擎分配内存的硬上限。每个类族调用一次 IDEA 原生入口；ZIP 按固定顺序由单线程写入。只提前提交与工作线程数相同数量的批次，完成结果的源码缓冲预算为 32 MiB，超出时暂存磁盘，消费后释放；取消会停止批内后续任务、等待工作线程退出并清理临时文件。该预算不包含 IDEA 原生反编译器和 VFS 的内存。

不再保留插件自己的源码缓存，避免字节码相同但 IDEA 反编译设置变化后返回旧结果。每次导出均调用原生入口；IDEA 自身的缓存行为由 IDEA 管理。

性能对比可运行 `./gradlew test --tests '*ExportBenchmarkTest' -PbenchmarkExports=true`。测试启动真实 IDEA 测试应用，对 1600 个 class / 800 个类族的嵌套 JAR 比较串行、逐类族并行、40 类族分批并行；交替三轮并核对源码、条目顺序及来源映射一致。耗时仅报告，不作通过阈值。扫描对比继续由 `ScanBenchmarkTest` 覆盖。

原生入口版本已在 IDEA Ultimate 2024.1.6 的真实测试应用中通过 61 项回归测试及逐字源码对比。上述 800 类族嵌套 JAR 在 CI 的三轮中位耗时：串行 3746 ms，逐类族并行（3 线程）1881 ms，40 类族分批（3 线程）2040 ms，40 类族分批（8 线程）1799 ms；源码、ZIP 条目顺序与来源映射一致。这些是该测试环境的观测值，不保证其他项目获得相同加速。262 版本的实际 IDE 应用仍需运行验证。
