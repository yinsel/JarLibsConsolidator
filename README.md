...
- **自动**（默认）：以 CPU 核数 × 2 为上限，扣除当前已用堆内存，并额外预留 `max(512 MiB, 最大堆的 25%)` 给 IDEA，再按每个任务 256 MiB 估算并发数，最少 1 个。实际线程数还受批次数限制。
- **高并发**：使用 CPU 核数 × 2 个任务；需要为 IDEA 配置足够堆内存。
- **低内存**：单线程逐类族反编译并写入。

自动模式在导出启动时估算并发，不会强制 GC；估算不是引擎内存硬上限。反编译器仍需在内存中构建当前类族并返回完整源码，扫描索引及 IDEA/VFS 也占内存。提高并发会增加当前任务的工作内存，但插件不累计已完成的源码。`idea.log` 会记录实际工作线程数和批大小。
...
性能对比可运行 `./gradlew test --tests '*ExportBenchmarkTest' -PbenchmarkExports=true`。测试启动真实 IDEA 测试应用，对 1600 个 class / 800 个类族的嵌套 JAR 比较串行、旧默认并发数、逐类族并行、40 类族分批并行、自动并发以及 CPU × 2 并发；交替三轮并核对源码、相对路径及来源映射一致（报告行顺序不参与比较）。旧默认并发模式用于隔离并发数量差异，调度器均为当前实现。耗时仅报告，不作通过阈值。扫描对比继续由 `ScanBenchmarkTest` 覆盖。
