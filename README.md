...
### 导出 CLASS / 反编译导出 JAVA（文件夹 / ZIP）

先在项目视图右键执行 **一键添加依赖**，将要分析的 JAR/class 登记到 IDEA 依赖库；然后选择 **一键导出 CLASS（文件夹 / ZIP）** 或 **一键反编译并导出 JAVA（文件夹 / ZIP）**，输入白名单、黑名单，选择输出格式，再选择导出父目录。默认模式自动创建 `<项目名>-classes` 或 `<项目名>-java-sources` 文件夹，沿用原 ZIP 的基本名称但不带 `.zip`；ZIP 模式则追加 `.zip` 扩展名；已有同名输出时使用 `-2`、`-3` 等后缀，不覆盖旧导出。
...
JAVA 导出直接调用 **当前 IDEA 编辑器注册的 Java Bytecode Decompiler 的 `getText(VirtualFile)` 入口**，传入原始 class/JAR 条目的 VirtualFile。反编译选项、用户设置、内部类读取、超时与引擎版本均由当前 IDEA 决定。插件不再直接构造底层引擎，不改写字节码、不关闭泛型重试、不修改返回源码，也不保留独立源码缓存。需启用 IDEA 自带反编译插件，并先在 IDEA 打开 class 完成其原生提示；插件不会代为接受提示。这里的一致指同一 IDEA 版本、设置和原始输入的原生反编译结果，不保证还原原始源码或所有方法均成功。

同一来源的内部类按类族调度，源码保持 IDEA 的原生合并结果。IDEA 自动读取的同目录 `$` 前缀 class 必须全部已选中；若它会读取被过滤或未登记的类，插件跳过该源码文件并记入 CSV，避免夹带未选中内容。需要导出此文件时，请将整个类族纳入白名单并移除对应黑名单。嵌套 JAR 原样暂存至任务结束，仍用 IDEA 的 JarFileSystem 读取，不改变内部条目或字节码。
...
普通条目按包路径导出，例如 `com/example/User.class` 或 `com/example/User.java`。相同完整类名和相同字节内容去重；同名但内容不同的版本全部保留在 `classes-conflicts/来源名称--字节摘要/` 下，不会互相覆盖。仅大小写不同的完整类名也分别放入冲突目录，以兼容 Windows 文件系统。JAVA 导出按整个类族的原始字节码判断版本：即使外部类字节相同，只要内部类内容不同，两套源码仍分别保留。反编译失败的版本会列入报告。

文件夹内包含 `export-report.txt`（扫描、过滤、去重、导出计数和失败/警告列表）以及 `export-sources.tsv`（导出文件对应的来源）。CLASS 使用小缓冲逐文件复制；JAVA 每个源码文件反编译完成后立即写入。报告和失败 CSV 也增量写入。导出可取消，取消或写入失败会保留已经完成的文件，删除正在写入的临时文件，并在报告中标记未完成；此时来源和失败报告可能尚未完整汇总。为限制异常输入的资源消耗，单个 class 上限 64 MiB，嵌套 JAR 上限 512 MiB、最多递归 8 层，超出部分会报告为失败/警告。

导出完成提示分别显示“成功导出：N 个完整文件”和已输出文件总数。JAVA 的部分源码不计入完整成功，另行提示部分源码数量及“反编译失败：N 个”。这里统计未生成源码的选中类版本，以及包含原生失败占位的源码文件；普通日志警告和扫描失败不混入该计数。N 大于 0 时，同一个文件夹额外包含 `decompilation-failures.csv`（UTF-8 BOM），列出完整类名、来源文件/JAR 条目、字节码 SHA-256、计划导出路径、异常类型和原因链；同名不同字节码分别记录。N 为 0 不生成该 CSV，CLASS 导出也不生成。CSV 支持逗号、双引号和多行异常；公式样式的字段会加前导单引号，完整原始消息仍保存在文本报告中。

IDEA 可能在日志中报告方法失败，同时返回其余源码。插件原样保留这份源码，不插入自定义警告头、不伪造失败方法；从原生 `$FF` 失败占位注释识别部分源码，在完成提示及 CSV 中记录输出文件和行号。原生公开入口不提供结构化的方法异常，因此 CSV 不猜测具体失败内部类或异常类型，详细堆栈仍查看 `idea.log`。IDEA 自身的引擎缺陷仍可能存在；在 IDEA 能看到一个类，不等于该类的每个方法都成功还原。

### 可选 ZIP 输出

CLASS 和 JAVA 导出对话框新增“输出格式”：默认仍为文件夹，也可以选择 ZIP 压缩包。选择导出父目录后，ZIP 命名为 `项目名-classes.zip` 或 `项目名-java-sources.zip`；同名输出已存在时使用 `项目名-classes-2.zip` 等序号，不覆盖原文件。包路径、冲突目录、过滤规则、来源报告和失败 CSV 与文件夹输出一致。

ZIP 使用快速压缩并逐个写入条目，不在内存中构建整个压缩包，也不累计所有源码。反编译保持现有并发调度；每个工作线程最多暂存一个输出文件到临时磁盘，完成后互斥写入 ZIP 并立即删除临时文件。源码不会进入额外结果队列。报告增量写入临时文件，结束时追加到 ZIP。压缩缓冲及 ZIP 条目目录会占用少量额外内存，条目越多目录信息越大；单个类的反编译仍需要内存。

取消在条目之间生效，正在提交的完整条目会先写完，随后尽量正常封装 ZIP，并在 `export-report.txt` 标记未完成。磁盘或压缩包写入故障导致 ZIP 损坏时删除无效压缩包。若 IDE 被强制结束或系统崩溃，ZIP 可能无法打开；需要随时直接查看已导出文件时建议选择文件夹。

### 反编译导出性能

JAVA 导出每批 40 个类族（外部类及选中的内部类），批内顺序、批间并行。工作线程完成一批后自行领取下一批；结果按完成顺序消费，慢类不会阻止其他线程继续调度。每个结果携带原始任务编号，确保冲突路径、来源及失败 CSV 对应正确的类。报告行顺序随完成顺序变化，文件命名和内容规则保持不变。

工作线程调用一次 IDEA 原生入口后直接写入对应 `.java` 文件，不等待整批完成，也不把源码放入 Future、结果队列或全量字符串列表。共享结果队列最多保留 `工作线程数 × 40` 个诊断结果，只有警告/失败信息；队列满时暂停领取更多工作。来源报告、失败 CSV 持续写入，仅为完成对话框保留前 20 条提示和完整计数。

JAVA 导出自动选择并发，移除并发输入框和固定默认 8 的设置。扫描完成后，按照 CPU 逻辑核数减一（至少一个）、IDEA 当前堆余量、系统可用物理内存及批次数决定初始工作线程数。每秒重新检查资源，在进入反编译接口前限制实际活动任务数；内存压力增大时暂停新任务，已有任务继续完成，恢复后可回升到本次导出的初始线程上限。每批仍为 40 个类族，因此不足 40 个类族时最多一个线程。

新版 Java Bytecode Decompiler 提供可选的 `getRecommendedParallelism()` 容量建议，JarLib 自动读取，让导出并发与当前引擎及单进程堆配置联动。原版 IDEA / 旧版插件没有此接口时，根据 IDEA 堆余量按每个任务 256 MiB 估算；有引擎建议时，另外按每任务 64 MiB 预留 JarLib 文本与 I/O 空间。IDEA 堆预留至少 512 MiB / 最大堆的四分之一（取较大者），系统预留至少 1 GiB / 总物理内存的 10%（取较大者）。至少串行执行一个任务，未知系统内存时退为串行；这些是调度估计，不能保证任意复杂类不耗尽内存。

`idea.log` 记录资源估计、引擎建议和实际线程数。为了两边容量联动，需要同时安装新版插件；较旧的反编译插件仍可能有自身的固定进程限制。源码仍只通过原生同步 `getText()` 获取，扫描索引、当前类族和 IDEA/VFS 仍占内存，插件不累计已完成源码。

参考 [Jar Analyzer 的分批索引与源码导出调度](https://github.com/yinsel/jar-analyzer/blob/8eb89ec4af191f9da5ea96af0561eff0719a2365/src/main/java/me/n1ar4/jar/analyzer/engine/index/IndexPluginsSupport.java)：40 个 class 是调度分组，并不是一次引擎调用反编译 40 个。该项目还复用同一次反编译结果进行索引和源码导出，批量添加索引后统一提交；JarLib 没有 Lucene 索引流程，也没有第二次源码导出反编译。JarLib 保持 IDEA 原生入口和内部类合并，不引入参考项目的源码 LRU 缓存或其独立引擎，因此相同调度不能保证不同引擎、不同输入下耗时完全一致。

写入失败会停止其他任务；取消会停止后续调度并等待工作线程退出，然后释放扫描临时文件。文件夹模式逐文件保留输出；ZIP 模式逐条写入压缩包，取消时尽量封装已完成条目。

不再保留插件自己的源码缓存，避免字节码相同但 IDEA 反编译设置变化后返回旧结果。每次导出均调用原生入口；IDEA 自身的缓存行为由 IDEA 管理。

性能对比可运行 `./gradlew test --tests '*ExportBenchmarkTest' -PbenchmarkExports=true`。测试启动真实 IDEA 测试应用，对 1600 个 class / 800 个类族的嵌套 JAR 比较串行、旧默认并发数、逐类族并行、40 类族分批并行、当前自动并发以及 CPU × 2 并发；交替三轮并核对源码、相对路径及来源映射一致（报告行顺序不参与比较）。旧默认并发模式用于隔离并发数量差异，调度器均为当前实现。耗时仅报告，不作通过阈值。扫描对比继续由 `ScanBenchmarkTest` 覆盖。

回归测试使用 IDEA Ultimate 2024.1.6 的真实测试应用，验证导出源码与原生入口逐字一致，并覆盖逐文件可见性、取消清理、磁盘失败、路径冲突和 CSV 增量写入。262 版本的实际 IDE 应用仍需运行验证。
...
DEBUG 记录扫描跳过的路径/异常、源文件到目标路径的映射、库根、导出过滤条件与导出路径，不记录字节码或反编译源码内容。分享日志前可遮盖敏感路径。导出单项失败的异常类型仍全部保存在 文件夹内 `export-report.txt`；为限制损坏归档产生的大量日志，每次导出默认只记录前 20 个单项失败的完整堆栈，其余堆栈在 DEBUG 下记录。取消操作不会当作失败报错。

反编译优化参考了 [jar-analyzer 的源码导出与索引流程](https://github.com/yinsel/jar-analyzer/tree/feat/flexible-class-filter-patterns) 中复用源码结果及同时处理内部类的思路；本插件继续使用 IDEA 随附引擎和有界任务队列，不添加另一套反编译器或索引。
