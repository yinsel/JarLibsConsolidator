...
JAVA 导出使用有限并行：根据 CPU 数量保留一个逻辑核心，最多 4 个工作线程（单/双核至少使用 1 个线程），每个任务保留独立反编译器上下文。每个类族只反编译一次，直接把内存中的结果写入 ZIP，不产生临时 Java 源码再读取。只提前处理与线程数相同数量的类族，ZIP 按固定顺序写入，避免无界排队和同名版本互相覆盖。
