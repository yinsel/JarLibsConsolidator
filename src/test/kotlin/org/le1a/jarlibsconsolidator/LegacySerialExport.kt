...
internal object LegacySerialExport {
    ...
    fun export(
        ...
        try {
            ...
            ZipOutputStream(Files.newOutputStream(archive)).use { zip ->
                for ((index, item) in items.sortedWith(compareBy<Item> { it.name }.thenBy { it.origin }).withIndex()) {
                    checkCanceled()
                    ...
                    val source = try {
                        decompiler?.decompile(item.file, item.name, checkCanceled)?.also { result ->
